...
第三套规则包如果局中真的要摇，加回来不到五十行，设计留在 `avalon/SCARS.md`
...
完整记录见 [`avalon/SCARS.md`](avalon/SCARS.md)。分三类：
...
- **触发队列泛化得比它的名字好。** 刺杀阶段不在静态阶段环里，用
  `NewAbilityTriggerEffect(谁, 哪个阶段)` 排进去，内核不但带过去了，还把
  **胜负判定推迟到它结算之后**——正是阿瓦隆要的。文档里它叫「死亡技能触发」。
...
阿瓦隆撞出来的疤 2、3（见 [`avalon/SCARS.md`](avalon/SCARS.md)）是同一个根因：
...
- **回合边界与胜负判定守同一条**：还有待结算的触发时不能落下。理由不同但都硬
...
- **整局结束不算新回合。** 旧代码 `nextPhase(PhaseEnd, startPhase())` 因为
  ...
  「碰巧」，改结构会连碰巧一起改掉。
- **出口优先级**：待结算的触发队列 > `GOTO_PHASE` > `NextPhase`。这条一开始只写
...
变异验证：拆掉 GOTO 的读取（阿瓦隆 3 个测试红）、把 GOTO 挪到触发队列之前
...
阿瓦隆撞出六条疤、对照 boardgame.io 又添一条（[`avalon/SCARS.md`](avalon/SCARS.md)、
...
**行动者那条抽象已经被逃逸三次**：狼人杀的猎人开枪（内核为它开了触发队列这个
...
**`Alive` 一个字段都没删。** 查下来真问题是自相矛盾：触发队列那条路死人可以行动
