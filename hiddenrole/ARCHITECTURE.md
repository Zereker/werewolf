...
- **枚举按名字序列化**（`"NIGHT_GUARD"` 而不是 `21`）。存档要给人看、也可能被别的
  语言读，编号对不上号。第三方的自定义取值没有名字，按编号写。
...
3. 构造时用 `hiddenrole.WithResolver(phase, resolver)` 注册解析器
   （`werewolf.New` / `NewWith` / `Restore` / `Replay` 与内核的
   `NewEngine` / `RestoreEngine` / `ReplayEngine` 都接受）
4. `hiddenrole.WithRoleSetup` 给出它的初始状态（阵营、类别、道具）

扩展点全部住在内核包 `github.com/Zereker/hiddenrole`——扩展规则
