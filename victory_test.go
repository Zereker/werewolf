...
type loversWin struct {
	a, b  string
	inner hiddenrole.VictoryChecker
}
...
func TestVictoryChecker_ThirdCamp(t *testing.T) {
	...
	g := newRuleGameWith(t, nil, []EngineOption{hiddenrole.WithVictoryChecker(checker)},
...
func TestWithVictoryChecker_RejectsNil(t *testing.T) {
	if _, err := hiddenrole.NewEngine(nil, hiddenrole.WithVictoryChecker(nil)); err == nil {
