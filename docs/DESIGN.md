...
| `RoleSystem` + `SkillAnnounce` | 「这一步没有玩家承担」——就绪判定不数它。不是「主持人」这个身份，那是规则包给它起的名字（`werewolf.RoleGod`） | ✅ |
...
| ~~**身份入座定死**~~ | **判错了，已撤销。** 第三套规则包（一夜狼人，正是这里点名的那个）验证的结果是反的：换牌类游戏要的不是「可写的 `RoleType`」，而是**两层身份**——发到手的那张（决定夜里做什么，不变）与现在手上那张（决定结算算哪边，会变）。内核给一层、规则给一层正好够用；压成一层，抢劫者就会跟狼一起醒，游戏当场垮掉。**不可变反而是这里的价值。** 见 [onenight/SCARS.md 疤 0](../onenight/SCARS.md) | — |
...
还要过一遍 [`SCARS.md`](../onenight/SCARS.md) 开头那条判据（**能绕过去的，
说明它是人机工程问题，不是抽象问题**），以及「破坏性签名变更要不要等第二次
撞上」。理由见下一节和 [ROADMAP.md §0](ROADMAP.md)。
